...
template <typename T>
class ExposedInput : public barrett::systems::System,
                     public barrett::systems::SingleInput<T> {
public:
  explicit ExposedInput(barrett::systems::ExecutionManager *em,
                        const std::string &sysName = "ExposedInput")
      : barrett::systems::System(sysName), barrett::systems::SingleInput<T>(
                                               this) {
...
public:
  BackgroundStatePublisher(barrett::systems::ExecutionManager *em,
                           barrett::systems::Wam<WAM_DOF> &wam,
                           haptic_wrist::HapticWrist *hw = nullptr)
      : wam(wam), hw(hw), stop_thread(false),
        nh((WAM_DOF == 4) ? "leader" : "follower"), exposedGravity(em),
        exposedExternalTorque(em) {
    ...
    external_torque_pub =
        nh.advertise<wam_teleop::GravityTorque>("external_torques", 1);
  ...
  ExposedInput<jt_type> exposedExternalTorque;
...
private:
  ...
  wam_teleop::GravityTorque externalTorque;
  ...
  void run() {
    ...
    while (ros::ok() && !stop_thread.load(std::memory_order_relaxed)) {
      ...
      jt_type ext_torque = exposedExternalTorque.getValue();
      for (size_t i = 0; i < WAM_DOF; i++) {
        ...
        externalTorque.torque[i] = ext_torque[i];
      }
#ifdef BUILD_LEADER
      if (WAM_DOF == 4 && hw != nullptr) {
        ...
        for (size_t i = 0; i < 3; i++) {
          ...
          externalTorque.torque[WAM_DOF + i] = ext_torque[WAM_DOF + i];
        }
      }
#endif
      ...
      externalTorque.header.stamp = ros::Time::now();
      external_torque_pub.publish(externalTorque);
